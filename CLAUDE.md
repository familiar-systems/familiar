...
- **Single-apex architecture collapses cookie scope.** The Astro site, SPA, and platform API all share one origin per environment (`localhost:8080` in dev, `preview.familiar.systems` in preview, `familiar.systems` in prod). Any cookie set at the apex is visible to all three services. Today nothing sets one: Hanko's session cookie (if the Hanko backend sets it at all) lives on Hanko's own domain (`auth.preview.familiar.systems`), not ours, and auth tokens travel as `Authorization: Bearer` headers. Before adding any cookie (analytics, preferences, feature flags, anything), think about the shared-cookie-space implication and scope it narrowly with `Path=` or drop it on a dedicated subdomain.
