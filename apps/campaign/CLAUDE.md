...
- `POST /campaign/{id}/pages`: Hanko-authenticated, GM-only (403 otherwise; role checked on the platform tier via `check_membership`). Creates a Page of any kind via one **kind-tagged union** request/response (`{ kind, content }`, adjacently tagged over `PageKind`). Common content fields: optional `status` (default `gm_only`), optional `parent` (a `PageId` to nest under in the ToC; omitted = ToC root, unknown = 422).
  - `kind: "entity"` / `kind: "template"`: document pages (`preamble` + `body`); `name` required (blank = 422). Driven by the supervisor's `CreatePage` (`kind`-selected `NewEntity` / `NewTemplate` genesis via `DbCreatePage`). Entity content also takes `from_template_id` (entity clone), accepted but 501 (not built). Returns 201 + the kind's response variant (`id`/`name`/`status`/timestamps; entity also carries `template_id`).
  - `kind: "session"`: a `kind = session` Page (prep/summary/transcript/journal) **and** its temporal `sessions` row (ordinal = max+1, the relationship FK target), minted together in one genesis transaction. `name` optional (blank/absent = unnamed, identified by ordinal). Driven by the supervisor's `CreateSession` workflow via the `PageActor`'s `NewSession` genesis path. Returns 201 + `{ kind: "session", content: { page_id, session_id, ordinal, name, created_at } }`.
