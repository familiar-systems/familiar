...
**Room actors.** `TocActor` (eager singleton) restores the ToC tree from `toc_entries`; `PageActor` (lazy, one per `PageId`) restores a page's blocks from `blocks WHERE section = 'content'`. Both reconstruct their Loro doc on start, mark dirty on `ClientUpdate`, debounce, then flush a full snapshot to `DatabaseWriteActor` (`WriteTocSnapshot` / `WritePageBlocks`); both flush on stop if dirty.
