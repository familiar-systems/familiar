...
- `POST /campaign/{id}/pages`: Hanko-authenticated, GM-only (403 otherwise; role checked on the platform tier via `check_membership`). Creates a Page of any kind via one **kind-tagged union** request/response (`{ kind, content }`, adjacently tagged over `PageKind`). Common content fields: optional `status` (default `gm_only`), optional `parent` (a `PageId` to nest under in the ToC; omitted = ToC root, unknown = 422).
  - `kind: "entity"` / `kind: "template"`: document pages (`preamble` + `body`); `name` required (blank = 422) and unique per kind (duplicate = 409). Driven by the supervisor's `CreatePage` (a `DocumentPageKind`-selected `NewDocumentPage` genesis via `DbCreatePage`). Entity content also takes `from_template_id` (entity clone), accepted but 501 (not built). Returns 201 + the kind's response variant (`id`/`name`/`status`/timestamps; entity also carries `template_id`).
  - `kind: "session"`: a `kind = session` Page (prep/summary/transcript/journal) **and** its temporal `sessions` row (ordinal = max+1, the relationship FK target), minted together in one genesis transaction. `name` required (blank = 422) and unique among sessions (duplicate = 409), like every page kind; the client renders `Session {ordinal}: {name}`. Driven by the supervisor's `CreateSession` workflow via the `PageActor`'s `NewSession` genesis path. Returns 201 + `{ kind: "session", content: { page_id, session_id, ordinal, name, created_at } }`.
  - The owning `PageActor` persists the Page and places it in the ToC; nothing writes the rows from the handler. See "CRDT rooms & collaboration".
...
| `POST /campaign/{id}/pages` (create any kind: entity/template/session) | `src/routes/pages.rs` |
...
Each file under `tests/` compiles as its own integration binary: `catalog_test`, `initialize_test`, `internal_init_test`, `lifecycle_test`, `metadata_test`, `schema`, `spike` (pages/blocks FK round-trip + vec-search GM/player filtering), and `wire_roundtrip` (loro-protocol framing). `config.rs` env-var tests are an inline `#[serial]` module (from `serial_test`), not a `tests/` file. ToC CRDT convergence is a unit test inside `src/loro/toc.rs`. `schema.rs` checks that the sea-orm entities match the schema the migrations build (self-contained; `cargo test` runs it, nothing to regenerate): the migrated user-table set equals the entity set, and each entity's columns (by name + SQLite type affinity), foreign keys, and unique constraints (incl. PK) match the migrated table, with an `Entity::find().all()` smoke per entity. CHECK constraints, non-unique (performance) indexes, defaults, and the vec0 table aren't compared (entities can't express them); they're migration-owned and, where load-bearing, behaviorally tested. A failure is a real entity/migration mismatch, not brittleness.
...
- [`docs/plans/2026-06-23-entity-relationship-temporal-model.md`](../../docs/plans/2026-06-23-entity-relationship-temporal-model.md): relationship schema, sessions-as-knowledge-time, retcon/supersede lifecycle.
