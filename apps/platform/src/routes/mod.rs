...
pub(crate) fn origin_matches(allowed: &str, origin: &str) -> bool {
    if let Some(suffix) = allowed.strip_prefix("https://*.") {
        origin.strip_prefix("https://").is_some_and(|rest| {
...
mod tests {
    ...
    fn exact_match_works() {
        assert!(origin_matches("http://localhost:5173", "http://localhost:5173"));
        assert!(!origin_matches("http://localhost:5173", "http://localhost:5174"));
        assert!(!origin_matches("http://localhost:5173", "https://localhost:5173"));
    }

    #[test]
    fn wildcard_matches_subdomain() {
        let allowed = "https://*.preview.familiar.systems";
        assert!(origin_matches(allowed, "https://app-pr-1.preview.familiar.systems"));
        assert!(origin_matches(allowed, "https://api-pr-1.preview.familiar.systems"));
        assert!(origin_matches(allowed, "https://multi.level.preview.familiar.systems"));
    }
    ...
    fn wildcard_rejects_http_scheme() {
        let allowed = "https://*.preview.familiar.systems";
        assert!(!origin_matches(allowed, "http://app-pr-1.preview.familiar.systems"));
    }
    ...
    fn wildcard_rejects_suffix_extension_attack() {
        ...
        let allowed = "https://*.preview.familiar.systems";
        assert!(!origin_matches(allowed, "https://preview.familiar.systems.evil.com"));
        assert!(!origin_matches(allowed, "https://app.preview.familiar.systems.evil.com"));
    }
    ...
    fn wildcard_rejects_prefix_match_only() {
        ...
        let allowed = "https://*.preview.familiar.systems";
        assert!(!origin_matches(allowed, "https://xpreview.familiar.systems"));
    }
}
