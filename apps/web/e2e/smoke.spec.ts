...
test("create a campaign, edit two pages, and navigate between them via the ToC", async ({
  ...
  // --- 1. Hub (empty on a fresh DB) -> start the first campaign. ---
  await page.goto("/");
  await page.getByTestId("start-first-campaign").click();

  // --- 2. Wizard (lands on /c/{id}, wizard-incomplete). ---
  ...
  // Picking Daggerheart auto-selects its template bundle, satisfying step 2.
  ...
  // --- 4. Edit the home page: two paragraph blocks. ---
  await typeTwoLines(page, page.locator(".ProseMirror"), "Home line one", "Home line two");

  // --- 5. Create a second page ("Test page") via the ToC. ---
  ...
  // --- 6. Edit the test page: two paragraph blocks. ---
  await typeTwoLines(page, page.locator(".ProseMirror"), "Test line one", "Test line two");

  // --- 7. Navigate back to home via the ToC; its content survived. ---
  ...
  // --- 8. Navigate to the test page via the ToC; its content survived too. ---
