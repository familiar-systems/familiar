...
- **Campaign-as-file isolation.** Each campaign is a self-contained SQLite database file.
...
- **Nothing happens without checkout.** The SQLite file must be on local disk.
...
- **Database-per-campaign on SQLite** (with sqlite-vec for vector search and sea-orm as the ORM). Each campaign is an isolated file. No shared multi-tenant database. Turso Database is the identified upgrade path.
...
Every campaign is a self-contained SQLite database file. There are no cross-campaign queries, no shared tables, no joins between campaigns. A campaign's database contains block records, entity data, relationships and graph edges, search text and embeddings, suggestion outcomes, agent conversation history, and campaign-specific metadata. During active editing sessions, LoroDoc state is held in actor memory as transient CRDT plumbing.
...
Object storage (Hetzner Object Storage) is the authoritative location for all campaign database files. Local disk on each application server is a working cache. The storage abstraction is [`CampaignStore`](../../apps/campaign/src/persistence/store.rs) with implementations for [local dev](../../apps/campaign/src/persistence/store_local.rs) and [S3](../../apps/campaign/src/persistence/store_s3.rs); the database lifecycle is managed by [`CampaignDatabase`](../../apps/campaign/src/persistence/database.rs).
...
1. `onLoadDocument`: client opens a page → Hocuspocus calls hook → hook reads relational data from SQLite → `toYdoc()` reconstructs a Y.Doc → Y.Doc is held in Hocuspocus memory → clients sync via Yjs protocol
...
Unchanged from the superseded ADR. The long pole in rendering a campaign view is the campaign checkout - downloading the SQLite file from object storage on a cold start. A SPA loads instantly from Bunny CDN, shows a loading skeleton, and handles the async checkout gracefully. SSR would block the entire page render waiting for the same operation. The SPA architecture decouples static asset serving (CDN) from data serving (campaign-pinned servers).
