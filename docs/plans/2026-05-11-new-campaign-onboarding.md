...
| **Internal-API auth is a shared bearer token, sourced from Scaleway Secrets Manager.** Both `/internal/init` (platform → campaign) and `/internal/campaigns/<id>/metadata` (campaign → platform) are protected by middleware that constant-time-compares the `Authorization: Bearer <token>` header against a process-startup-read secret. The token is distinct per environment (prod, preview, local-dev). Pulumi references the secret by path, never by value; rotation is "update secret, roll both deployments." | Two services, low call volume, layer-2 threat boundary. Bearer is honest about the scale: one secret, two readers, constant-time compare on the receive side. mTLS via cert-manager is the textbook answer but pays in local-dev pain (Issuer + per-pod certs in `mise run dev`) and operational overhead that the threat model does not yet warrant. KMS-signed JWTs via Scaleway Key Manager are the more rigorous upgrade path (signing key never exfiltrable, per-call audit) and are a reasonable v0+1 reach if internal call volume or service count grows — but at two services on one cluster, the bearer's leak surface is bounded by "pod compromise already gives campaign DB access," which the bearer is not the load-bearing defense against. |
...
| `apps/campaign/src/routes/internal.rs`                          | new: `POST /internal/init { campaign_id, owner_user_id }`; idempotent via `INSERT OR IGNORE` on `campaign_metadata.id`                                                                                                                                       |
...
| `infra/` Pulumi configs                                         | add `/catalog/*` Traefik IngressRoute on PR-preview and prod                                                                                                                                                                                                 |
