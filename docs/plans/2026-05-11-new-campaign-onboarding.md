...
| **`POST /api/campaigns` is minimal.** Body is just an idempotency token; the platform allocates a `CampaignId`, creates the campaign on a shard (`POST /internal/campaign`), acquires its lease (`PUT /internal/campaign/{id}/lease`), writes the routing row, and returns `{ campaign_id }`.                                                                                                                                                                                                                                                                                                                                                                             | Keep the create call platform-only so the platform stays domain-blind. Bundling configuration into create would force the platform to gateway domain-shaped payloads (template slugs, locale, game system) it has no business interpreting. Configuration is the campaign tier's concern: it owns `PATCH /campaign/{id}`, which atomically commits everything when the GM seals the wizard.                                                                                                                                                                                                                                                                                                     |
| **The wizard runs in-campaign, not pre-campaign.** `POST /api/campaigns` creates a blank campaign and the SPA redirects into it; the wizard overlay renders while `wizard_completed_at IS NULL`.                                                                                                                                                                                                                                                                                                                                                                                  | The campaign tier owns the wizard's single write (`PATCH /campaign/{id}` with `wizard_complete: true`); the SPA renders the campaign route with the wizard overlaid; the redirect target is the actual campaign URL. Putting the wizard at a pre-campaign route would force the platform to gateway domain-shaped payloads it should not interpret.                                                                                                                                                                                                                                                                                                                                          |
| **The wizard is atomic at Seal, not incremental.** Wizard state is client-only across all steps. On Seal, the SPA fires one call to the campaign tier: `PATCH /campaign/{id}` carrying the full payload (game_system, content_locale, name, tagline, template_slugs) with `wizard_complete: true`. The handler validates required fields and runs everything in one SQLite transaction.                                                                                                                                                                                                                       | Modeling the wizard as N incremental writes leaks implementation convenience into the data model. A wizard is an initialization ceremony, not a stream of edits. The PATCH endpoint supports partial updates for future metadata edits (rename, tagline change) without the `wizard_complete` flag, but the wizard uses the atomic path. Atomicity erases sticky-field race conditions, template idempotency edge cases, and partially-configured intermediate states. Abandonment leaves the campaign blank, which the user can re-walk from scratch. |
...
| **Idempotency at every retry boundary.** Create-flow: the platform's `create_attempts` table maps `(idempotency_token -> campaign_id)`; `POST /internal/campaign` is idempotent via `INSERT OR IGNORE` on `campaign_metadata.id`; `PUT /internal/campaign/{id}/lease` is idempotent by nature; the routing-table insert is idempotent via PK. Wizard-flow: `PATCH /campaign/{id}` with `wizard_complete: true` is guarded by a `wizard_completed_at IS NULL` precondition; a retry against an already-committed campaign returns `409`.                                                                                                                         | Two retryable client calls (`/api/campaigns` and `PATCH /campaign/{id}`), two idempotency strategies. The create flow uses three PK constraints so retries no-op cleanly. The wizard flow uses a precondition gate: the SPA's Seal retry on 5xx is safe because the only way a retry can land "again" is if the first attempt did not commit, in which case `wizard_completed_at` is still NULL and the second call proceeds. If the first attempt did commit, the second returns 409 and the SPA dismisses. No status columns, no reconciliation pass.                                                                                                                                           |
...
> **Data structures and type signatures in this section are illustrative, not final.** Field names, struct shapes, where meta lands (SQL columns vs Loro containers), and similar concrete choices are placeholders to make the prose readable — they will be revisited at implementation time. Treat them as "something with this rough shape" rather than a contract.
...
SPA
  ...
  `-- Wizard overlay (shown while campaign_metadata.wizard_completed_at IS NULL).
     All steps are client-side state. Nothing hits the server until Seal:
       ...
       step 4: review + Seal
       seal: PATCH /campaign/{id} --> Campaign tier
                { game_system,                  validates everything, then in one
                  ...
                  wizard_complete: true }        (PATCH /internal/platform/campaign/{id})
                                                with the changed fields. The server
                                                stamps wizard_completed_at on commit.

       On failure (any slug doesn't resolve, transaction aborts, etc.) the
       campaign tier fires POST /internal/platform/campaign/{id}/init-failed
...
`/internal/*` endpoints on both tiers (platform's `PATCH /internal/platform/campaign/{id}` + `POST /internal/platform/campaign/{id}/init-failed`, campaign's `POST /internal/campaign` + `PUT /internal/campaign/{id}/lease`) gate through three layers. Each layer carries its own role and its own failure mode; the bearer in §Decisions is the third, not the load-bearing one.
...
5. `POST <shard>/internal/campaign { campaign_id, owner_user_id }`. Shard does `INSERT OR IGNORE INTO campaign_metadata(id, owner_user_id)` and returns 200. Idempotent.
5b. `PUT <shard>/internal/campaign/{id}/lease`. Ensures the campaign is loaded. Idempotent.
...
| `content/templates/common/*.yaml`, `content/templates/<system-id>/*.yaml` | Use the new node type in any template that needs it (e.g. `content/templates/common/npc.yaml`, `content/templates/blades-in-the-dark/crew.yaml`). |
...
- **Input**: a parsed `Template { meta, body }` (post-`serde_yaml::from_str`) plus a target locale.
- **Output**: a `CompiledTemplate { name, description, icon, doc }`. `name` and `description` are `meta.*` resolved to the chosen locale (with `en` fallback for missing keys). `icon` is `meta.icon` as-is (locale-independent). `doc` is a `LoroDoc` whose `doc` container holds a ProseMirror-shaped tree of the body, also locale-resolved. Together these are everything the instantiation route needs to persist as a new Thing. How meta fields land — SQL columns on `things`, Loro containers alongside `doc`, or some mix — is a persistence-layer decision deferred to implementation.
...
apps/campaign/src/starter_content/
  ...
  template.rs           # Template { meta: TemplateMeta, body: TemplateBody }; parses .yaml
  schema.rs             # JSON Schema codegen for the template DSL
  compile.rs            # Template -> CompiledTemplate (structural constants only)
...
```rust
fn compile(template: &Template, locale: &Locale) -> CompiledTemplate

struct CompiledTemplate {
    name: String,        // meta.name resolved for `locale`
    description: String, // meta.description resolved for `locale`
    icon: String,        // meta.icon (locale-independent)
    doc: LoroDoc,        // ProseMirror-shaped tree of the body
}
```
...
- `structure_hash`: sha256 over a canonical-serialized form of the parsed `Template.body` tree (node types, attributes, nesting). Locale-independent; meta is not part of structure.
- `content_hash`: sha256 over `(structure, locale-resolved strings from both meta and body)`. Locale-specific.
...
```
YAML --parse--> Template --compile(locale)--> CompiledTemplate --canonicalize--> CanonicalForm --sha256--> seeded_*_hash
Thing (persisted meta + LoroDoc) --materialize--> CanonicalForm --sha256--> current_*_hash
```
...
All fields are optional on the wire type (`PatchCampaignRequest`), but when `wizard_complete` is true the server validates that `name`, `game_system`, and `content_locale` are present (either in the request or already persisted). The same endpoint supports future metadata edits without the `wizard_complete` flag.
...
**Idempotency / retry.** When `wizard_complete: true`, the endpoint is guarded by the `wizard_completed_at IS NULL` precondition. A second call after a successful Seal returns `409 Conflict`. A retry after a network failure that didn't commit will succeed because `wizard_completed_at` is still NULL. The SPA's retry policy on Seal is: on 5xx, retry the same payload; on 4xx (other than 409), surface the error; on 2xx, dismiss the overlay. **On 409, treat as success and dismiss the overlay.** PATCHes without `wizard_complete` are always idempotent (partial updates no-op when the value is unchanged).
...
**Post-Seal view is intentionally minimal in v0.** Once the wizard-complete PATCH commits and the overlay dismisses, the campaign view just renders what's in the database as text: name, tagline, game system, content locale, and the ToC's Things by name. No metadata editing UI, no template-adding UI, no in-place renaming. Settings, post-wizard template addition, and any edit-in-place behavior are deferred (see §Out of scope). A future session builds on top of this.
...
Template instantiation in v0 has exactly one caller: the Seal handler (`PATCH /campaign/{id}` with `wizard_complete: true`), which invokes it once per `template_slug` inside the init transaction. There is no in-campaign "add a template later" route in v0 (see §Out of scope).
...
- `name: Option<String>` (source of truth; mirrored to platform; written by PATCH)
- `tagline: Option<String>` (source of truth; mirrored to platform; written by PATCH)
- `game_system: Option<String>` (opaque display string; source of truth; mirrored to platform; written by PATCH. The wizard resolves the catalog entry's locale-resolved `name`, the trimmed BYO custom name, or the frontend's BYO empty-input fallback, and ships exactly that. Neither tier interprets this field.)
- `content_locale: Option<String>` (BCP-47; source of truth; mirrored to platform; written by PATCH; sticky — see §Localization)
- `audio: Option<AudioMode>` (NULL until wizard-complete PATCH commits; required at Seal; sum type `opt-in | opt-out | text-only`; not mirrored to platform — privacy choices stay on the campaign tier)
- `evals_enabled: Option<bool>` (NULL until wizard-complete PATCH commits; required at Seal; not mirrored to platform)
- `wizard_completed_at: Option<DateTime<Utc>>` (NULL until wizard-complete PATCH commits; server-stamped, not from the SPA payload; sticky once set; gates the wizard overlay; mirrored to platform so the SPA can route without a shard fetch)
...
- **Adding templates after the wizard.** In v0, templates are only instantiated through the atomic wizard-complete PATCH. There is no in-campaign "add a template from the catalog" affordance. The data model supports it (Things with `is_template = true` can be created at any time), but no route, no UI, and no test exercises it in v0.
...
| Path                                                            | Change                                                                                                                                                                                                                                                       |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `content/systems.yaml`                                          | exists (catalog of supported systems); add a row per new system. Existing rows: `dnd-5e`, `daggerheart`, `blades-in-the-dark`.                                                                                                                               |
| `content/templates/common/*.yaml`, `content/templates/<system-id>/*.yaml` | template files; existing: `content/templates/common/npc.yaml`, `content/templates/common/player.yaml`, `content/templates/blades-in-the-dark/crew.yaml`. Add as v0 widens.                                                                                |
...
| `crates/app-shared/src/campaigns/internal.rs`                   | Rust-only platform<->campaign internal wire types (`CreateCampaignRequest`, `PatchCampaignMirror`, `InitFailedRequest`); both binaries import from here so the shape can't drift                                                                                                   |
| `crates/campaign-shared/src/onboarding/`                        | `catalog.rs` (`CatalogResponse`, `SystemEntry`, `TemplateRef`) + `initialize.rs` (`PatchCampaignRequest`, `CampaignErrorResponse`, `AudioMode`); ts-rs-exported to `packages/types-campaign/src/generated/onboarding/`                                    |
| `apps/platform/src/routes/campaigns.rs`                         | `POST /api/campaigns`: upsert `create_attempts`, mint `CampaignId`, call shard `POST /internal/campaign` + `PUT /internal/campaign/{id}/lease`, insert routing row, return `{ campaign_id }`. `GET /api/campaigns`: return the authenticated user's campaign rows from the mirrored columns. |
| `apps/platform/src/routes/internal_campaigns.rs`                | `PATCH /internal/platform/campaign/{id}` receiving metadata mirror updates from the campaign tier; `POST /internal/platform/campaign/{id}/init-failed` recording a failure reason on the routing row. Bearer middleware mounted on `/internal/platform/*`. |
...
| `apps/campaign/src/routes/internal.rs`                          | `POST /internal/campaign` (create with owner) + `PUT /internal/campaign/{id}/lease` (idempotent checkout); bearer-token middleware on `/internal/campaign/*` (see §Internal-API defense layers). |
...
| `apps/campaign/src/routes/metadata.rs`                          | `GET /campaign/{id}` (read metadata) + `PATCH /campaign/{id}` (partial update; when `wizard_complete: true`, validates required fields, sets `wizard_completed_at`, runs template instantiation, fires platform mirror) |
...
| `Caddyfile.dev`                                                 | add `/catalog/*` -> campaign-tier routing via `handle_path` (strip-prefix). Binary serves `/systems` post-strip, not `/catalog/systems`. Same StripPrefix expected on the prod Traefik IngressRoute. |
...
- `apps/campaign` PATCH happy-path test: `PATCH /campaign/{id}` with `wizard_complete: true` and a valid payload (game_system, content_locale, name, tagline, template_slugs). Assert `campaign_metadata` rows are set, one Thing per slug exists with `is_template = true`, `seeded_locale` matches, `seeded_structure_hash` and `seeded_content_hash` match the build-time table, ToC has one new entry per slug, `wizard_completed_at` is set, and the platform receives one PATCH mirror call.
- `apps/campaign` PATCH atomicity test: inject a per-slug compile failure on the second of three template_slugs; assert the transaction rolls back and the call returns a structured error.
- `apps/campaign` PATCH wizard_complete precondition test: call `PATCH /campaign/{id}` with `wizard_complete: true` twice; assert the second call returns `409` and no state changes.
- `apps/campaign` PATCH without wizard_complete test: send a partial update (e.g., just `name`); assert the name changes, `wizard_completed_at` remains NULL, and the platform receives a mirror PATCH.
...
- `apps/campaign` create idempotency test: call `POST /internal/campaign` twice with the same campaign_id; assert the second call no-ops and returns 200; only one `campaign_metadata` row exists.
- `apps/campaign` lease test: call `PUT /internal/campaign/{id}/lease` for an existing campaign; assert 200.
- `apps/platform` create-call test: `POST /api/campaigns` with a stub shard. Verify a `create_attempts` row is written, the shard is called (POST create + PUT lease), the routing row is inserted, and `{ campaign_id }` is returned.
