...
| `internal-bearer-prod`                 | Shared bearer token gating `/internal/*` endpoints between the prod platform and campaign tiers. Deployed as the prod `internal-bearer` k8s Secret with key `INTERNAL_BEARER_PRIMARY`. **Pulumi-minted and Pulumi-managed end-to-end** (see `cloud.py::internal_bearer_prod_*`). |
| `internal-bearer-preview`              | Same role as `internal-bearer-prod`, scoped to every preview namespace. Identical value across all open PRs (preview is shared trust). Consumed by `ci_cd_preview.yml` via `fetch-scw-secret` and materialized into a per-namespace `internal-bearer` k8s Secret alongside the rest of the preview manifests. **Pulumi-minted and Pulumi-managed end-to-end.** |
...
The `internal-bearer-*` secrets are different from the other operator-bootstrapped credentials: they have no external origin. The value is just `random_bytes` — Pulumi mints it with `pulumi_random.RandomPassword`, writes it into SM as a `scaleway.secrets.SecretVersion`, and feeds the same `Output` directly into the k8s `Secret` in `k8s.py`. No `read_secret` indirection, no operator step, nobody ever sees the value in plaintext. The "never wire IaC to rotatable credentials" rule applies specifically to credentials feeding `replaceOnChanges` fields on cascading parents (like `kubernetes.Provider.kubeconfig`); these bearers feed `kubernetes.core.v1.Secret.string_data`, which is replace-triggering but does not cascade, so Pulumi ownership is safe.
