...
| `internal-bearer-prod`                 | Shared bearer token gating `/internal/*` endpoints between the prod platform and campaign tiers. Deployed as the prod `internal-bearer` k8s Secret with key `INTERNAL_BEARER_PRIMARY`. Operator-rotated; see rotation contract below. |
| `internal-bearer-prod-secondary`       | **Transient.** Present only during a rotation window for `internal-bearer-prod`. Holds the incoming bearer value so both tiers accept old-and-new simultaneously during the swap. Removed after rotation completes. |
| `internal-bearer-preview`              | Same role as `internal-bearer-prod`, scoped to every preview namespace. Identical value across all open PRs (preview is shared trust). Consumed by `ci_cd_preview.yml` via `fetch-scw-secret` and materialized into a per-namespace `internal-bearer` k8s Secret alongside the rest of the preview manifests. |
...
The `internal-bearer-*` secrets are operator-bootstrapped, not Pulumi-managed:

```bash
# One-time, per environment (prod + preview). Generate a random 32-byte
# value, base64-encode, and write to SM. Pulumi reads it at apply time.
scw secret secret create name=internal-bearer-prod \
    region=fr-par
scw secret version create internal-bearer-prod \
    data="$(openssl rand -base64 32)" \
    region=fr-par

scw secret secret create name=internal-bearer-preview \
    region=fr-par
scw secret version create internal-bearer-preview \
    data="$(openssl rand -base64 32)" \
    region=fr-par
```

This mirrors the operator-bootstrap model used for `familiar-systems-*-key` (the SM container exists outside Pulumi; Pulumi reads the value at apply time). The bearer is rotatable, and the "never wire IaC to rotatable credentials" rule applies: the value flows into a non-cascading k8s `Secret` (`Opaque`), not into the `kubernetes.Provider.kubeconfig` field, so rotation replaces one resource without triggering the cascade.

### Rotation: `internal-bearer-prod`

The bearer is symmetric (same value sent and verified by both platform and campaign). Both tiers' middleware accepts a set of valid bearers: `INTERNAL_BEARER_PRIMARY` (always present) plus an optional `INTERNAL_BEARER_SECONDARY` (present only during rotation). Senders use only the primary. The rotation procedure eliminates the rolling-deploy window where some pods hold the old value and others hold the new:

1. **Generate a new value:** `NEW="$(openssl rand -base64 32)"`.
2. **Stage the new value as SECONDARY:** create `internal-bearer-prod-secondary` SM secret and write `$NEW` to it.
3. **`pulumi up`** — the prod `internal-bearer` k8s Secret now carries both `INTERNAL_BEARER_PRIMARY` (old) and `INTERNAL_BEARER_SECONDARY` (new). Both Deployments roll over; both pods accept old and new. Senders still send the old PRIMARY.
4. **Swap in SM:** write `$NEW` to `internal-bearer-prod`, write the old value to `internal-bearer-prod-secondary`.
5. **`pulumi up`** — Secret data flips; pods roll over; senders flip to the new PRIMARY; receivers still accept the old via SECONDARY.
6. **Retire the old value:** delete the `internal-bearer-prod-secondary` SM secret.
7. **`pulumi up`** — Secret data drops the SECONDARY key; rotation complete.

Each `pulumi up` produces one `Secret` replace (the empirical behavior of `kubernetes.core.v1.Secret.data` changes documented in §Registry pull credential applies here too). The replace is confined to this one Secret — no provider cascade. The brief sub-second gap where the Secret doesn't exist only affects newly scheduling pods (which retry on `CreateContainerConfigError`); pods already running keep the env values they read at startup.

The same procedure applies to `internal-bearer-preview`, with the difference that preview pods are short-lived and the operator may prefer to skip the SECONDARY dance entirely (just write a new value, redeploy each preview). For a single-tenant preview namespace, the rolling-deploy window is the entire fleet — one Deployment per tier — so the SECONDARY dance is overkill there. Operator's call.
