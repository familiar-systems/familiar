...
| `internal-bearer-prod`                 | Shared bearer for prod platform ↔ campaign `/internal/*`. Operator-filled; Pulumi reads "latest" via `config.read_secret`. The SM container is Pulumi-managed (`cloud.py::internal_bearer_prod_secret`), the value isn't. |
| `internal-bearer-preview`              | Same role as `internal-bearer-prod`, scoped to every preview namespace. Identical value across all open PRs (preview is shared trust). Read by `ci_cd_preview.yml` via `fetch-scw-secret` and materialized into a per-namespace `internal-bearer` k8s Secret. |
...
The `internal-bearer-*` secrets follow the same pattern as `bunny-api-key`: Pulumi creates an empty SM container; the operator fills it with one command. The bearer value has no external origin (unlike Bunny / Hetzner Console / k3s SA token), so the "one command" is `openssl rand -base64 32` piped into `scw secret version create`. The operator never sees the value.

### Bootstrap: `internal-bearer-prod` / `internal-bearer-preview`

One command per environment, run after the initial `pulumi up` has created the empty SM containers:

```bash
openssl rand -base64 32 | scw secret version create internal-bearer-prod   data=- region=fr-par
openssl rand -base64 32 | scw secret version create internal-bearer-preview data=- region=fr-par
```

`pulumi up` (prod) and the next `ci_cd_preview.yml` run (preview) pick up the value via `revision="latest"`.

### Rotation

Same command. SM versions accumulate; consumers always read `latest`.
...
`pulumi up` reads the new `latest`, the prod `internal-bearer` k8s Secret's `string_data` changes (replace-triggering on Secrets, per §Registry pull credential), and the `checksum/internal-bearer` annotation on the campaign pod template changes (`bearer_checksum` in `k8s.py`). The campaign Deployment rolls automatically; the platform Deployment is not affected in this MR because it doesn't yet read the bearer (envFrom on platform lands with the route-implementation MR that adds the middleware). When that MR lands, this annotation gets mirrored onto the platform pod template and rotation rolls both tiers.

Preview rotation: same `scw secret version create` against `internal-bearer-preview`. Each open PR's next workflow run fetches the new value, recomputes `INTERNAL_BEARER_CHECKSUM` in the deploy step, and the annotation update rolls each PR's campaign Deployment.
